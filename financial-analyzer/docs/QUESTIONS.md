...
| **Q64** | **`δ_MEU`, `f_min` et `J_min` par cellule** — surplus minimal utile, plancher de fréquence, contribution économique minimale par unité de temps. Les trois entrent directement dans les niveaux A, B et C de l'exclusion oracle. Sans eux, la phase 0 ne peut ni exclure ni conclure (ADR-192). | 🔴 | Q51-A add.2 §4 |
| **Q65** | **Politique de chevauchement et capacité** : `DISJOINT_WINDOWS` ou `CAPACITY_CONSTRAINED_ORACLE`, cooldown, concurrence maximale, contrainte de capital. Détermine combien de tirages indépendants le système obtient réellement — un même mouvement ne doit pas compter autant de fois qu'il produit d'horodatages (ADR-193). | 🟠 | Q51-A add.2 §8-9 |
...
| **Q59-A** | **Méthode d'inférence** — architecture retenue : `ANYTIME_VALID_WHEN_QUALIFIED` avec repli `FIXED_HORIZON` **par cellule** (ADR-197, ADR-198). Une cellule dont la dépendance n'est pas maîtrisée n'est pas perdue : elle change de protocole. Reste à trancher les tolérances de qualification (ACF, stationnarité) et à les déclarer avant la première observation. | ✅ moteur · 🔴 **tolérances** | Q51-A add. §1-6 · add.2 §21-30 |
