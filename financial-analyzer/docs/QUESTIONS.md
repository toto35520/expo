...
| **Q63** | **Plancher de coûts `C_floor`** — borne **inférieure** du coût réellement inévitable, **par cellule et par type d'ordre** (ADR-194). Ni estimation centrale, ni coût prudent : `C_réel ≥ C_floor` doit être défendable. Le spread n'entre pas dans le plancher d'un ordre passif ; les crédits sont signés ; un composant estimé entre par sa borne inférieure. Sans lui, l'exclusion oracle ne se calcule pas. | 🔴 | Q51-A add.2 §11-15 |
...
| **Q65** | **Contraintes `HARD` contre `POLICY`** : chaque cooldown, limite de concurrence et type d'ordre doit être classé. Seules les contraintes dures entrent dans le `PHYSICAL_ORACLE` — un cooldown arbitraire y fabriquerait une exclusion imputable à notre architecture, pas au marché (ADR-206). | 🔴 | Q51-A add.3 §12-13 |
