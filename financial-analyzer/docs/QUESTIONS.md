...
| **Q63** | **Plancher de coûts `C_floor(h, c)`** — le coût aller-retour le plus bas plausible, par horizon et par cellule. Sans lui, l'exclusion oracle ne se calcule pas. Relève de Q40 et du modèle de coûts, pas d'une croyance sur le signal. | 🔴 | Q51-A add. §7 |
...
| **Q59-A** | **Méthode d'inférence** — `FIXED_HORIZON` ou `ANYTIME_VALID`, à trancher **avant** les valeurs et avant la première observation. Les deux sont implémentées et testées par simulation adverse (ADR-181, ADR-182). Recommandation : `ANYTIME_VALID`, dont la couverture est vérifiée ; à défaut `FIXED_HORIZON` pour la première campagne — un protocole plus simple mais valide vaut mieux qu'un protocole sophistiqué mal calibré. | ✅ moteur · 🔴 **décision** | Q51-A add. §1-6 |
