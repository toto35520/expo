...
| --- | --- | --- | --- |
| **Q1** | **Structure figée** (ADR-213, `docs/Q1-cible-economique.md`) : cible primaire = valeur nette ajustée du risque, `NO TRADE` en décision de première classe ; `J_min` cible, `δ_MEU` plancher de matérialité, `f_stat_min` condition de validabilité, `f_econ_min` dérivée. **Restent à déclarer** : objectif chiffré, unité de performance, capital de référence, tolérance de risque, horizon d'évaluation, et surtout le **rôle** — alerte, recommandation ou exécution. Question distincte, inchangée : emplacement du code et pile technique. Le dépôt hôte est TypeScript/React Native ; les étages 5, 7 et 12 (régime, fusion, calibration) vivent naturellement en Python. Service autonome, tout en TS, ou noyau numérique séparé ? | 🔴 | ADR-005 |
...
| **Q64** | **Seuils économiques dérivés de Q1** — `δ_MEU`, `J_min`, et **deux** planchers de fréquence à ne pas confondre : statistique et économique. Une cible principale est déclarée, les autres en dérivent, et la redondance est signalée. **Ne bloque pas la collecte**, seulement le verdict (ADR-207, ADR-204). | 🔴 *(après Q1)* | Q51-A add.3 §14-17 |
