...
| --- | --- | --- | --- |
| **Q1** | **Devenu préalable au premier verdict** (ADR-207) : objectif économique, unité de performance, capital de référence, tolérance de risque, horizon d'évaluation et rôle du système doivent être fixés avant de dériver Q64. Sans eux, `δ_MEU` et `J_min` ne seraient que des nombres commodes. Reste aussi : emplacement du code et pile technique. Le dépôt hôte est TypeScript/React Native ; les étages 5, 7 et 12 (régime, fusion, calibration) vivent naturellement en Python. Service autonome, tout en TS, ou noyau numérique séparé ? | 🔴 | ADR-005 |
