...
| --- | --- | --- | --- |
| ~~**Q1**~~ | ~~Cible économique~~ — **figée** : `Q1-GOLD-RECOMMENDATION-V1`, empreinte `c1127d72f9fcced6` (ADR-245). Rôle `RECOMMENDATION`, unité `R` avec `1R` = 0,50 % de l'equity, `J_min` = +0,10 R/séance sur 60 séances, `δ_MEU` = +0,20 R, risque ≤ 1R, simultané ≤ 2R, perte ≤ 12R. | ✅ | ADR-245 |
| **Q1-b** | **Pile technique et emplacement du code** — question distincte, sans effet sur les seuils. Le dépôt hôte est TypeScript/React Native ; les étages 5, 7 et 12 vivent naturellement en Python. Service autonome, tout en TS, ou noyau numérique séparé ? | 🟠 | ADR-005 |
...
| **Q50** | **Export de cotations bid/ask horodatées** — le **collecteur est branché** : `feasibility/mt5_source.py` + `collect_xauusd.py` produisent la timeline depuis le terminal MT5 (ADR-260 à 263). Ce qui manque n'est plus du code : c'est de **lancer la collecte** sur le compte réel. | ✅ collecteur · 🔴 exécution | Adaptateur §1 |
| **Q51** | **Journalisation de latence** — moteur implémenté (ADR-149 à 160) **et branché** : les cinq points d'instrumentation tournent dans `collect_xauusd.py`, validés de bout en bout sur rejeu. Ce qui manque : **la première séance réelle**. Ces données ne se reconstruisent pas après coup. | ✅ moteur · ✅ branchement · 🔴 séance | Adaptateur · Q19 §5 · Q51 |
| ~~**Q61-A**~~ | ~~Budget signal-agnostique~~ — **résolu** : la borne oracle `U_capture(L, h, c)` se calcule sans aucun signal, direction connue d'avance et sortie parfaite comprises. `U_capture ≤ C_floor` exclut la cellule (ADR-183). Ne dépend que de `C_floor`, qui relève de Q40. | ✅ | Q51-A add. §7 |
| **Q61-B** | **Latence admissible propre à un moteur** `L_max,j(h, c)`, dérivée de `edge_j`. Spécifique au moteur, à l'horizon, à la cellule, au régime et au type d'ordre ; figée dans le protocole du moteur avant son évaluation finale. **Ne bloque pas la phase 0** — l'y imposer réintroduirait une croyance sur l'alpha dans un test conçu pour en être indépendant (ADR-183, ADR-184). | 🟠 *(après signal)* | Q51-A add. §8 |
| ~~**Q62**~~ | ~~Ancrage de la capturabilité~~ — **résolu par typage** : trois ancres et trois portées distinctes, jamais fusionnées, chacune imposant son nom de publication ; la fin d'horizon est déclarée séparément pour ne pas créer un second estimande en silence (ADR-185, ADR-186). Une exclusion `POST_RECEIVE_ONLY` conclut ; sa non-exclusion ne qualifie rien. | ✅ | Q51-A add. §10-13 |
| **Q63** | **Plancher de coûts `C_floor`** — **spécifié et figé `PROVISIONAL`** : `Q63-XAUUSD-ICMARKETS-MT5-V1` (ADR-255 à 259). Structure complète, composants non prouvés à `0`, financement `UNRESOLVED` hors rollover prouvé. Manquent trois éléments que seul le terminal donne — `ACCOUNT_TYPE`, `ACCOUNT_BASE_CURRENCY`, `MT5_SYMBOL_SPECIFICATION` — lus automatiquement par `--preflight-only`. **Ne bloque pas la collecte** : un plancher non résolu interdit une exclusion, pas une observation. | ✅ structure · 🟠 3 relevés | Q51-A add.2 §11-15 |
| **Q66** | **Qualification de couverture du bootstrap** — l'estimateur est implémenté et exécuté, avec ses tests de calibration. Ce qui manque : la **campagne** établissant `P(p ≤ p_U) ≥ 1 − α` sous les dépendances réellement observées, et le domaine où elle est revendiquée. Les tests de calibration **mesurent** la couverture actuelle — 0,937 à 0,960 selon `p`, dont la borne inférieure n'atteint pas la cible. Aucun certificat n'est donc émissible en l'état (ADR-237, ADR-238). | ✅ moteur · 🔴 calibration | Q51-A add.4 §1 · add.5 §2 |
| **Q68** | **Borne supérieure d'espérance `EV_U`** par famille de moteurs. Seule construction permettant d'exclure économiquement un moteur **rare** : `f_nécessaire = (J_min + C) / EV_U`. Sans elle, la rareté ne peut pas être opposée à la qualité, et `δ_MEU` ne s'y substitue jamais (ADR-234). | 🟠 | Q51-A add.5 §11 |
| **Q67** | **Borne physique du gain `S_U`** par cellule et horizon, sous forme de **dérivation recalculable** (ADR-227) : déplacement maximal qu'un survivant pourrait capturer, avec ses entrées et leur provenance. Dérivable de l'export Q50. | 🟠 | Q51-A add.4 §3 |
| ~~**Q64**~~ | ~~Seuils économiques~~ — **dérivés de Q1-v1** (ADR-247) : `EconomicFrequencyRequirement.from_mandate()` lit `J_min` et le divise par la borne d'espérance, en portant version et empreinte du mandat. Reste ouvert : `f_stat_min`, qui vient du protocole de puissance et non de l'économie. | ✅ *(sauf f_stat_min)* | Q1-v1 |
| **Q64-bis** | **Seuils économiques dérivés de Q1** — `δ_MEU`, `J_min`, et **deux** planchers de fréquence à ne pas confondre : statistique et économique. Une cible principale est déclarée, les autres en dérivent, et la redondance est signalée. **Ne bloque pas la collecte**, seulement le verdict (ADR-207, ADR-204). | 🔴 *(après Q1)* | Q51-A add.3 §14-17 |
| ~~**Q65**~~ | ~~Contraintes `HARD` contre `POLICY`~~ — **figée** : `Q65-GOLD-RECOMMENDATION-V1`, empreinte `d712142fbf1603bb` (ADR-251 à 254). 17 `HARD`, 12 `POLICY`, registre **fermé** — l'inconnu bloque au lieu d'être deviné. Le capital est conditionnel : prouvé, il limite la portée au compte ; non prouvé, il est écarté. Le `PHYSICAL_ORACLE` reçoit tous les types d'ordre du courtier. | ✅ | ADR-251 |
...
| ~~**Q59-A**~~ | ~~Méthode d'inférence~~ — **tranchée** : `FIXED_HORIZON` pour la première campagne normative, `ANYTIME_VALID_EXPERIMENTAL` calculé en parallèle sans valeur normative (ADR-203). Le passage à `ANYTIME_VALID` attend un argument **positif** de validité sous la dépendance réelle — les diagnostics ACF et stationnarité n'y suffisent pas (ADR-202). | ✅ | Q51-A add.3 §7-8 |
| **Q59-E** | **Estimande principal** : `EVENT_WEIGHTED` ou `CLUSTER_WEIGHTED` ? Dépend de la sémantique des futurs moteurs — au plus une décision par rafale, ou une par événement. La phase 0 publie les deux ; le choix du principal reste ouvert (ADR-195). | 🟠 | Q51-A add.2 §18-19 |
| **Q59-B/C/D** | **Couverture minimale, précision utile, règle d'arrêt.** Le coût de chaque direction est chiffrable d'avance (`clusters_for_separation`) : exclure demande ~70 grappes, conclure « non exclu » ~1 100. Aucun seuil arbitraire n'est proposé — la suffisance dépendra de la variance réellement observée (ADR-175). | ✅ moteur · 🔴 valeurs | Q51 §28 · Q51-A §15 |
